...
\begin{thm}\label{thm:kummer}
	...
	This finishes the case in which $C$ has finite index over $\wp(A_{K})$, and the general case follows from this case thanks to \Cref{lm:coefficients}, because
	...
	We want to reduce to the case of finite cyclic field extensions in order to apply \Cref{axiom}.
	...
	By \Cref{lm:finiteduality} it is in turn enough to show that the induced homomorphism
	\begin{align*}
	...
	\Cref{axiom} implies that $g$ is a $1$-coboundary, i.e.~there exists some $a\in A_{L}$ such that $g(\sigma)=a-\sigma(a)$ for all $\sigma\in \Gal(L/K)$.
...
Let us skip the necessary checks now and go straigth to the application of the theorem for $\bbF_{p}$ and exponent $p$.
Let's fix an algebraic closure $\overline{\bbF}_{p}$ and take $A:=\overline{\bbF}_{p}$ and
\begin{align*}
    \wp\colon \overline{\bbF}_{p} & \longrightarrow \overline{\bbF}_{p} \\
    a & \longmapsto a^{p}-a.
\end{align*}
We have then $\mu_{p}=\bbF_{p}=A_{\bbF_{p}}$ and $\wp(A_{\bbF_{p}})=0$, so according to \Cref{thm:kummer}, there should be a single degree $p$ extension of $\bbF_{p}$ inside $\overline{\bbF}_{p}$, corresponding to the subgroup $\bbF_{p}\trianglelefteq \bbF_{p}$.
...
Let $\alpha\in \overline{\bbF}_{p}$ be a root of $t^{p}-t-1$.
